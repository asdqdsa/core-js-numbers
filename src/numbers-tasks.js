...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const v1v2 = x1 * x2 + y1 * y2;
  const v1 = Math.hypot(x1 ** 2, y1 ** 2);
  const v2 = Math.hypot(x2 ** 2, y2 ** 2);
  return Math.acos(v1v2 / (v1 * v2));
}
...
function getSumOfDigits(num) {
  if (num < 1) return 0;
  return getSumOfDigits(Math.floor(num / 10)) + (num % 10);
}
